...
export const generateClassicInvoicePDF = (data: InvoiceData): jsPDF => {
  ...
  yPos += 8;
  const paymentInfoY = yPos;
  doc.setDrawColor(0);
  doc.rect(margin, paymentInfoY, pageWidth - 2 * margin, 30);
  
  // Header
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  addText('PAYMENT INFORMATION', margin + 3, paymentInfoY + 4);
  
  // Dividing line
  doc.line(margin + (pageWidth - 2 * margin) / 2, paymentInfoY + 6, margin + (pageWidth - 2 * margin) / 2, paymentInfoY + 30);
  
  // Left Column - Bank Name & Account Number
  const colWidth = (pageWidth - 2 * margin) / 2;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  addText('Bank Name:', margin + 5, paymentInfoY + 10);
  doc.setFont('helvetica', 'normal');
  addText(data.bankName || 'N/A', margin + 30, paymentInfoY + 10);
  
  addText('Account No:', margin + 5, paymentInfoY + 17);
  doc.setFont('helvetica', 'normal');
  addText(data.accountNumber || 'N/A', margin + 30, paymentInfoY + 17);
  
  // Right Column - IFSC Code & Account Holder
  doc.setFont('helvetica', 'bold');
  addText('IFSC Code:', margin + colWidth + 5, paymentInfoY + 10);
  doc.setFont('helvetica', 'normal');
  addText(data.ifscCode || 'N/A', margin + colWidth + 30, paymentInfoY + 10);
  
  doc.setFont('helvetica', 'bold');
  addText('A/C Holder:', margin + colWidth + 5, paymentInfoY + 17);
  doc.setFont('helvetica', 'normal');
  addText(data.accountHolder || 'N/A', margin + colWidth + 30, paymentInfoY + 17);
  
  yPos = paymentInfoY + 32;

  // Add Signature Section
  yPos += 8;
  const signatureSectionY = yPos;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  addText("RECEIVER'S SIGNATURE", margin + 5, yPos);
  yPos += 5;
  
  // Signature line box
  doc.rect(margin, yPos, thirdWidth - 10, signatureBoxHeight);
  yPos += signatureBoxHeight + 3;
  
  // Name field
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  addText('Name: _________________', margin, yPos);
  yPos += 5;
  
  // Date field
  addText('Date: _________________', margin, yPos);
  
  // Proprietor's Signature Box
  yPos = signatureSectionY + 5;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  addText("PROPRIETOR'S SIGNATURE", margin + thirdWidth, yPos);
  yPos += 5;
  
  // Signature line box
  doc.rect(margin + thirdWidth, yPos, thirdWidth - 10, signatureBoxHeight);
  yPos += signatureBoxHeight + 3;
  
  // Name field
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  addText('Name: _________________', margin + thirdWidth, yPos);
  yPos += 5;
  
  // Date field
  addText('Date: _________________', margin + thirdWidth, yPos);
  
  // Company Stamp Box
  yPos = signatureSectionY + 5;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  addText('COMPANY STAMP', margin + 2 * thirdWidth + 10, yPos, { align: 'center' });
  yPos += 5;
  
  // Dashed border for stamp box
  const stampBoxX = margin + 2 * thirdWidth + 5;
  ...
  doc.setLineDash([2, 2]);
  doc.rect(stampBoxX, yPos, stampBoxWidth, stampBoxHeight);
  doc.setLineDash([]);
  
  // Add placeholder text
  doc.setFontSize(8);
  doc.setTextColor(150, 150, 150);
  addText('[Place Stamp Here]', stampBoxX + stampBoxWidth / 2, yPos + stampBoxHeight / 2, { align: 'center' });
