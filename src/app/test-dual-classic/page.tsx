...
export default function TestDualClassicPrintPage() {
  ...
  const [printMode, setPrintMode] = useState<'side-by-side' | 'duplex'>('side-by-side');

  // Mock invoice data (same as test-template)
  const mockInvoice: Invoice = {
    id: 'QTN001',
    invoiceNumber: 'QTN001',
    quotationDate: new Date('2025-08-25'),
    dueDate: new Date('2025-09-24'),
    customer: {
      name: 'Test Customer Ltd.',
      address: '123 Business Street, Commercial District, City - 400001',
      phone: '+91-9876543210',
    ...
    items: [
      {
        ...
        detailedDescription: 'High quality product with advanced features',
        quantity: 5,
        unit: 'PCS',
        rate: 1500.00,
        discount: 10,
      ...
      },
      {
        ...
        detailedDescription: 'Premium service offering',
        quantity: 2,
        unit: 'PCS',
        rate: 2500.00,
        discount: 5,
      ...
      },
      {
        ...
        detailedDescription: 'Standard item with warranty',
        quantity: 10,
        unit: 'PCS',
        rate: 750.00,
        discount: 0,
    ...
    totalAmountInWords: 'Eighteen Thousand Rupees Only',
  ...
  const handlePrint = () => {
    ...
    try {
      // Check if we're in a secure context and have access to window.print
      if (typeof window !== 'undefined' && window.print) {
        setTimeout(() => {
          try {
            ...
            // Fallback: Try to trigger print via document
            if (document.execCommand) {
              document.execCommand('print', false, null);
            } else {
              alert('Unable to print. Please use Ctrl+P (Windows) or Cmd+P (Mac)');
            }
          } finally {
            setPrinting(false);
          }
        ...
        setPrinting(false);
        alert('Print function is not available. Please use your browser\'s print option (Ctrl+P or Cmd+P)');
      }
    } catch (error) {
      console.error('Print setup error:', error);
      setPrinting(false);
      alert('Print function encountered an error. Please try using Ctrl+P or Cmd+P');
    }
  };
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Box>
                    <Typography variant="h5" sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                      <AutoAwesomeIcon />
                      🧪 Test Dual Classic Print
                    </Typography>
                    <Typography variant="body2" sx={{ opacity: 0.9 }}>
                      Invoice: {mockInvoice.invoiceNumber} • {printMode === 'duplex' ? 'Both Sides (Duplex)' : 'Side by Side'} • {printMode === 'duplex' ? '🔄 Double-sided' : '📄 Single-sided'}
                    </Typography>
                  </Box>
                </Box>
