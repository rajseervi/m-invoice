...
import { sanitizeForFirestore, validateUpdateDocData } from '@/utils/firestoreUtils';
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      const productData = {
        ...
        categoryName: finalCategory, // Store category name for backward compatibility
    ...
    try {
      ...
      await executeWithRetry(
        async () => {
          ...
          if (invoiceId) {
            // Update existing invoice - validate and clean data to prevent undefined values
            ...
            console.log('Updating invoice with validated data:', validation.cleanedData);
            
            const invoiceRef = doc(db, 'invoices', invoiceId);
            await updateDoc(invoiceRef, validation.cleanedData);
            currentInvoiceId = invoiceId;
          } else {
            // Create new invoice using centralized service with stock validation
            const createResult = await CentralizedInvoiceService.createInvoice(invoiceData, {
              validateStock: true,
              updateStock: true,
            ...
            if (!createResult.success) {
              throw new Error(createResult.blockingErrors?.join(', ') || createResult.errors?.join(', ') || 'Failed to create invoice');
            }
            
            currentInvoiceId = createResult.invoiceId!;
        ...
        <DialogContent>
          <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField
              ...
              helperText={validateProductName(newProductName || '') || 'No spaces. Use underscore, hyphen, or camelCase.'}
            />
