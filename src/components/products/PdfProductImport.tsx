...
const PdfProductImport: React.FC<PdfProductImportProps> = ({ open, onClose, onSuccess }) => {
  ...
  const extractProductData = (text: string): ExtractedProduct[] => {
    ...
    const patterns = [
      // Specific pattern for the provided invoice format:
      // Sl No. (optional) Description of Goods Amount Disc. % per Rate Quantity HSN/SAC
      // Example: "1   SWR S/S UG- Pipe 110mm SN-4 (10ft)   2,87,677.50 67 % pcs 1,585.00 550.00 pcs 39172390"
      {
        regex: /^\s*\d*\s*(.+?)\s+([\d,\.]+)\s+\d+\s*%\s+\w+\s+([\d,\.]+)\s+([\d,\.]+)\s+\w+\s+\d+/,
        nameIndex: 1,
        priceIndex: 3, // This is the 'Rate' column
        qtyIndex: 4,   // This is the 'Quantity' column
        confidence: 99,
        description: 'Specific Invoice Format (Sl No. Name Amount Disc% per Rate Quantity HSN/SAC)'
      },
      // Fallback to existing patterns if the specific one doesn't match
      // Specific pattern for the provided invoice format:
      // Sl No. (optional) Description of Goods Amount Disc. % per Rate Quantity HSN/SAC
      // Example: "1   SWR S/S UG- Pipe 110mm SN-4 (10ft)   2,87,677.50 67 % pcs 1,585.00 550.00 pcs 39172390"
      {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Try the specific invoice pattern first
      const specificMatch = line.match(patterns[0].regex);
      if (specificMatch) {
        console.log(`Specific Invoice Pattern matched:`, specificMatch);
        const nameText = specificMatch[patterns[0].nameIndex]?.trim();
        const priceText = specificMatch[patterns[0].priceIndex]?.trim();
        const qtyText = specificMatch[patterns[0].qtyIndex]?.trim();

        if (nameText && priceText && qtyText) {
          ...
          if (!isNaN(quantity) && !isNaN(price) && quantity > 0 && price > 0) {
            ...
            if (cleanName.length > 2) {
              ...
              productFound = true;
              console.log('Specific Pattern extraction:', { name: cleanName, quantity, price });
            }
          }
        }
      }

      // If specific pattern didn't work, try position-based extraction if we have column positions
      if (!productFound && columnPositions && headerIndex >= 0 && i > headerIndex) {
        try {
          ...
          // Improved position-based extraction: split by spaces and map to columns
          const parts = line.split(/\s{2,}/); // Split by 2 or more spaces

          // Find the part that corresponds to each column based on header positions
          const findPartForColumn = (columnName: string) => {
            const colIndex = sortedPositions.findIndex(pos => pos.name === columnName);
          ...
          nameText = findPartForColumn('name');
          qtyText = findPartForColumn('qty');
          priceText = findPartForColumn('price');

          // Fallback if simple splitting doesn't work perfectly
          if (!nameText || !qtyText || !priceText) {
            // Revert to substring if parts are not clearly separated
            ...
            if (!isNaN(quantity) && !isNaN(price) && quantity > 0 && price > 0) {
              ...
              if (cleanName.length > 2) {
                products.push({
                  ...
                  quantity,
                  price,
                  confidence: 90, // Slightly lower confidence for general position-based
                  rawText: line,
                  category: inferCategory(cleanName)
                });
                productFound = true;
      ...
      // If neither specific nor position-based extraction worked, try general pattern matching
      if (!productFound) {
        ...
        if (lineAnalysis.hasText && lineAnalysis.numberCount >= 2 && lineAnalysis.wordCount >= 2) {
          // General patterns (original patterns from before)
          const generalPatterns = patterns.slice(1); // Exclude the specific invoice pattern
  ...
  const processPdf = async () => {
    ...
    try {
      ...
      if (products.length === 0) {
        setError('No product data could be extracted from the PDF. Please ensure it contains a clear product list.');
        return;
      }
  ...
  const importProducts = async () => {
    ...
    try {
      ...
      for (const mapping of productMappings) {
        ...
        if (mapping.action === 'create') {
          productsToCreate.push({
            ...
            categoryName: mapping.newCategory || extractedProduct.category || 'General', // Use categoryName for service to handle creation
            price: extractedProduct.price,
            quantity: extractedProduct.quantity, // Use quantity for stock
            description: extractedProduct.description || '',
            // Add other default product fields as needed
          ...
          if (mapping.updateStock) {
            updateData.quantity = extractedProduct.quantity; // Update quantity for stock
          }
          ...
          if (Object.keys(updateData).length > 0) {
            await productService.updateProduct(mapping.existingProductId, updateData);
            updatedCount++;
          }
        }
      }
      ...
      if (productsToCreate.length > 0) {
        const result = await productService.importProducts(productsToCreate);
        createdCount = result.success;
        if (result.errors.length > 0) {
          setError('Some products failed to import: ' + result.errors.join('; '));
        }
      }
      
      setProcessingStatus(`Import complete: ${createdCount} created, ${updatedCount} updated`);
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  id="pdf-import-demo-mode-switch"
                  name="pdfImportDemoModeSwitch"
                />
              }
