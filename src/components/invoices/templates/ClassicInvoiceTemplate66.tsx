...
export default function ClassicInvoiceTemplate({ invoice, settings, previewMode, copyLabel }: ClassicInvoiceTemplateProps) {
  ...
  // Calculate scale factor for single page printing
  useEffect(() => {
    const calculateScale = () => {
      const template = document.querySelector('.tally-template');
      if (!template) return;

      const availableHeight = 281 * 3.78; // 281mm in pixels at 96dpi (281 * 96/25.4)
      const contentHeight = template.scrollHeight;
      
      if (contentHeight > availableHeight) {
        const scale = Math.min(1, availableHeight / contentHeight);
        setScaleFactor(scale);
        template.style.setProperty('--scale-factor', scale.toString());
      }
    };
  ...
  const printStyles = `
    ...
    @media print {
      * {
      ...
      /* Auto-scaling to ensure single page fit */
      .auto-scale {
  ...
  return (
    <React.Fragment>
      ...
      <div className="print-scale-container">
        <Box className="tally-template tally-border auto-scale" sx={{
          width: '100%',
          maxWidth: previewMode ? '210mm' : 'none',
          margin: '0 auto',
          p: previewMode ? 0.5 : 0,
          bgcolor: 'white',
          minHeight: previewMode ? '281mm' : 'auto',
          height: previewMode ? '281mm' : 'auto',
          maxHeight: previewMode ? '281mm' : 'none',
          ...
          <TableContainer>
            <Table className="tally-table" sx={{ border: '1px solid #000' }}>
              ...
              <TableBody>
                ...
                {/* Empty rows for spacing - Tally style (reduced for single page) */}
                {Array.from({ length: Math.max(0, Math.min(6, 8 - (invoice.items?.length || 0))) }).map((_, index) => (
