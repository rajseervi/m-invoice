          ...
          {/* Invoice Information */}
           ...
           <Grid item xs={6} sx={{ flex: '1', textAlign: 'left', width: '30%' }}>
            <Box className="tally-section-border tally-info-section" sx={{
              ...
              <Typography variant="body2" sx={{
                fontSize: (invoice.partyName && invoice.partyName.length > 20) ? '0.75rem' : '0.9rem',
                fontWeight: 'bold',
                mb: 0.1,
                overflow: 'hidden',
                        ...
                        (
                          (typeof item.totalAmount === 'number')
                            ...
                            : (typeof item.finalPrice === 'number')
                              ? item.finalPrice
                              : ((item.price || 0) * (item.quantity || 0) * (1 - ((item.discountType === 'percentage' ? (item.discount || 0) : 0) / 100)))
                        ) * (1 + ((typeof (item as any).margin === 'number' ? (item as any).margin : 0) / 100))
                      ).toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
                  ...
                  <TableCell className="number-cell" sx={{
                    ...
                    {formatCurrency(
                      (invoice.items || []).reduce((sum: number, item: any) => {
                        const base = (typeof item.totalAmount === 'number')
                          ? item.totalAmount
                          : (typeof item.finalPrice === 'number')
                            ? item.finalPrice
                            : ((item.price || 0) * (item.quantity || 0) * (1 - ((item.discountType === 'percentage' ? (item.discount || 0) : 0) / 100)));
                        const marginPct = typeof item.margin === 'number' ? item.margin : 0;
